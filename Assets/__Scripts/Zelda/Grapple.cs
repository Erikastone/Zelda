...
public class Grapple : MonoBehaviour
{
    ...
    public float grapleLength = 7;
    public float grapleInLength = 0.5f;
    public int unsafeTileHealthPanalty = 2;
    ...
    void StartGrapple()
    {
        ...
        dray.enabled = false;
        anim.CrossFade("Dray_Attack" + facing, 0);
    ...
    private void FixedUpdate()
    {
        switch (mode)
        {
            case eMode.gOut:// Крюк брошен
                ...
                if ((p1 - p0).magnitude >= grapleLength)
                {
            ...
            case eMode.gInHit:// Крюк зацепился, поднять Дрея на стену
                float dist = grapleInLength + grappleSpd * Time.fixedDeltaTime;
                if (dist > (p1 - p0).magnitude)
                {
                    p0 = p1 - (directions[facing] * grapleInLength);
    ...
    void StopGrapple()
    {
        ...
        if (mode == eMode.gInHit && unsafeTiles.IndexOf(tileNum) != -1)
        {
            ...
            dray.ResetInRoom(unsafeTileHealthPanalty);
        }
        grapHead.SetActive(false);
        mode = eMode.none;
    }
